...
`Vuex`是一个`Vue.js`的扩展库，是一个关于状态管理模式的库，提供的作用主要是用于Vue应用的全局数据管理。

安装*vue dev tool extension for chrome* 后，`Vuex`可以使用*Chrome DevTool*中进行调试。
